...
- **Автентифікація:** Flask-Login, модель `User`. Ролі: `is_admin` (boolean), `is_active_master` (boolean). Користувач є або адміністратором, або майстром (визначається `not User.is_admin`). Поле `schedule_display_order` (Integer, nullable=True, default=999) використовується для сортування активних майстрів у розкладі.
...
- **Управління майстрами:** Адміністратори можуть керувати статусом `is_active_master` та порядком `schedule_display_order` для майстрів через інтерфейс редагування користувачів у auth blueprint (шлях /auth/users/<id>/edit).
...
- Історія міграцій була виправлена через розв'язання проблеми з відсутньою ревізією та створенням міграції злиття (поточний HEAD: 65f9ff3f0dfd).
